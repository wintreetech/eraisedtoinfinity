...
function drawFooter(page, fonts) {
	...
	const { footerLineY, footerText, fontSize, paddingTop } =
		footerMetrics(fonts);
...
function drawTable(page, fonts, gridX, gridTop, gridW, headerH, rowH, rows) {
	...
	page.drawText(t2, {
		x:
			gridX +
			col1 +
			(col2 - fonts.semibold.widthOfTextAtSize(t2, headSize)) / 2,
		y: headY,
		size: headSize,
		font: fonts.semibold,
		color: c("#111827"),
	...
	page.drawText(t3, {
		x:
			...
			col1 +
			col2 +
			(col3 - fonts.semibold.widthOfTextAtSize(t3, headSize)) / 2,
		y: headY,
		size: headSize,
		font: fonts.semibold,
		color: c("#111827"),
	...
	rows.forEach((r, i) => {
		...
		drawRect(page, gridX, bottom, gridW, rowH, {
			borderColor: stroke,
			borderWidth: 0.8,
		});
...
function renderPage2(page, fonts, assets) {
	...
	function renderSection(title, body) {
		...
		const bodyLines = wrapText(
			...
			fonts.regular,
			12,
			layout.right - layout.left - sectionPad * 2
		);
